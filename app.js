...
function GTMCalculator() {
  ...
  const [editBudgetValue, setEditBudgetValue] = React.useState(20000);
          ...
          React.createElement('div', { className: 'bg-white rounded-xl shadow-lg p-6' },
            ...
            React.createElement('div', { className: 'mb-6' },
              React.createElement('div', { className: 'flex justify-between items-center mb-2' },
                React.createElement('label', { className: 'block text-sm font-medium text-slate-700' }, 'Monthly Budget'),
                React.createElement('button', {
                  ...
                  className: `px-3 py-1 rounded text-sm font-medium transition ${editingBudget ? 'bg-red-100 text-red-700 hover:bg-red-200' : 'bg-blue-100 text-blue-700 hover:bg-blue-200'}`
                }, editingBudget ? 'Cancel' : 'Edit')
              ),
              !editingBudget && React.createElement('div', { className: 'flex gap-2 mb-3' },
              ...
              editingBudget && React.createElement('div', { className: 'bg-blue-50 border border-blue-200 rounded-lg p-4 mb-3' },
                React.createElement('label', { className: 'block text-sm font-medium text-slate-700 mb-2' }, 'Enter Custom Budget'),
                ...
                React.createElement('button', {
                  onClick: () => {
                    const tiers = [20000, 30000, 40000, 50000];
                    const closestTier = tiers.filter(t => t <= editBudgetValue).sort((a, b) => b - a)[0] || 20000;
                    setBudget(editBudgetValue);
                    const profiles = editBudgetValue >= 50000 ? 3.5 : editBudgetValue >= 40000 ? 2.5 : editBudgetValue >= 30000 ? 2 : 1;
                    setOutreachProfiles(profiles);
                    setEditingBudget(false);
                  },
